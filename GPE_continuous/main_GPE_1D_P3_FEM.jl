...
# P3 Lagrage FEM for spatial discretization
#
...
#----- solve -----#
# generate mesh
Tri, nodes, nodes2mesh = get_triangulation_P3(Nx, x_a, x_b);

# read intial value
Nh = 2^16 # (coarse) spatial elements for intial value
u0_vec = evaluate_initial_value_P3(Tri, nodes, Nx, Nh)

# get system matricies
println("assemble system matrix")
tid = time();
A = assemble_stiffness_matrix_P3(Tri, nodes, nodes2mesh);
M = assemble_mass_matrix_P3(Tri, nodes, nodes2mesh);
M_V = assemble_potential_matrix_P3(V, Tri, nodes, nodes2mesh);
time_offline = time() - tid;

# time stepping
println("start time stepping")
tid = time();
u = solve_GPE_1D_q2_P3(A, M, M_V, alpha, beta, V, u0_vec, T, Nt, Tri, nodes, nodes2mesh, threshold, l_max);
...
u_h = polongate_solution_P3(u[:, Nt+1], Tri, nodes, nodes2mesh, nodes_h);
...
#----- save to MAT file-----#
file_name = "P3_Hlevel" * string(H_level);
