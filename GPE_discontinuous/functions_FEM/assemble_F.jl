function assemble_F(v, Tri, nodes, nodes2mesh)
    ...
    for i in eachindex(Tri[:, 1])
        ...
        if idx[1] != 0
            f[idx[1]] += detBT * (w[1] * f1 * phi1(tau[1]) + w[2] * f2 * phi1(tau[2]) + w[3] * f3 * phi1(tau[3]))
        end

        if idx[2] != 0
            f[idx[2]] += detBT * (w[1] * f1 * phi2(tau[1]) + w[2] * f2 * phi2(tau[2]) + w[3] * f3 * phi2(tau[3]))
        end
    end
